...
We use this decomposition on the previous condition formula together with the previous result on the Covariance of two elements of the sequence:
...
When used in this shape it is immediately recognizable the fact that a variance formula of the same form can be found in the Bernoulli distribution, for which the condition we found for the correlation equal to be equal to $1$ is true no matter what parameter $p \in [0,1]$ we choose, since:

$$
\theta \sim Ber(p), \;\;\;\;\;\; E[\theta] = p, \;\;\;\;\;\; Var[\theta]=p(1-p)
$$

A Bernoulli distribution also satisfies the fact that the distribution of $\theta$ has to be inside $[0, 1]$, but foregoes the continuity of the distribution.
